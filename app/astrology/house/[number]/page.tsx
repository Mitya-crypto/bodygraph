...
export default function HousePage() {
  ...
  const houseNumber = parseInt(params.number as string)
      ...
      <div className="min-h-screen bg-space-900 flex items-center justify-center">
        <div className="cosmic-card text-center">
          <p className="text-cosmic-300">
            {language === 'ru' ? 'Дом не найден' : 'House not found'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            <a 
              href="/results?module=astrology"
              className="cosmic-button bg-space-700 hover:bg-space-600 p-2"
            >
              <ArrowLeft className="w-5 h-5" />
            </a>
            <div>
              <h1 className="cosmic-title text-3xl flex items-center gap-3">
                <span className="text-4xl">{houseInfo.icon}</span>
                {houseInfo.title}
              </h1>
              <p className="text-cosmic-400 mt-1">
                {language === 'ru' ? 'Подробное описание астрологического дома' : 'Detailed astrological house description'}
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="cosmic-card">
            ...
            <div className="flex flex-wrap gap-3">
              {houseInfo.keywords.map((keyword, index) => (
                <span
                  key={index}
                  className="px-4 py-2 bg-cosmic-600/30 text-cosmic-300 rounded-full text-sm font-medium"
                >
                  {keyword}
                </span>
              ))}
            ...
            <ul className="space-y-3">
              {houseInfo.influences.map((influence, index) => (
                <li key={index} className="flex items-start gap-3">
                  <span className="text-cosmic-400 mt-1 text-lg">•</span>
                  <span className="cosmic-text text-base">{influence}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Совет */}
          <div className="cosmic-card bg-cosmic-600/20 border border-cosmic-600/30">
            <h3 className="cosmic-subtitle text-xl mb-3">
              {language === 'ru' ? 'Практический совет' : 'Practical Advice'}
            </h3>
            <p className="cosmic-text text-lg italic leading-relaxed">{houseInfo.advice}</p>
          </div>
        </motion.div>
      </div>
    </div>
