...
export default function HumanDesignStrategyByTypePage({ params }: PageProps) {
  ...
  // Преобразуем slug в тип и стратегию
  const resolvedParams = usePromise(params)
  const slug = decodeURIComponent(resolvedParams?.type || '').toLowerCase()
  ...
  const resolvedType = slugToType[slug] || humanDesignData.type

  const typeToStrategyKey: Record<string, string> = {
    'Generator': 'Respond',
  ...
  const strategyKey = typeToStrategyKey[resolvedType] || humanDesignData.strategy
    ...
    const descriptions = {
      ru: {
        ...
        'Inform': {
          ...
          benefits: ['Меньше сопротивления', 'Поддержка окружения', 'Покой', 'Успешная реализация']
        },
  ...
  const currentStrategyDescription = getStrategyDescription(strategyKey)
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          ...
          <ul className="list-disc list-inside text-cosmic-300 space-y-2">
            {currentStrategyDescription.howTo.map((item: string, index: number) => (
        ...
        >
          ...
          <ul className="list-disc list-inside text-cosmic-300 space-y-2">
            {currentStrategyDescription.examples.map((item: string, index: number) => (
              <li key={index}>{item}</li>
            ))}
          </ul>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {currentStrategyDescription.benefits.map((benefit: string, index: number) => (
