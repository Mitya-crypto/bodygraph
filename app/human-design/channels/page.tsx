...
export default function HumanDesignChannelsPage() {
  ...
  useEffect(() => {
    ...
    // Перенаправляем на страницу со всеми каналами
    router.replace('/human-design/channels/all')
  }, [userProfile, router])
    ...
    <div className="min-h-screen bg-space-900 flex items-center justify-center">
      <div className="cosmic-card text-center">
        ...
        <h2 className="cosmic-subtitle text-xl mb-2">
          {language === 'ru' ? 'Загрузка каналов...' : 'Loading channels...'}
        </h2>
        <p className="text-cosmic-300">
          {language === 'ru' ? 'Перенаправление на страницу каналов' : 'Redirecting to channels page'}
        </p>
      </div>
    </div>
  )
